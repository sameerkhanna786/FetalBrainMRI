...
| Source-data final lock         | Supplement / appendix             | Dovjak table check, Woitek implementation byte-check countersignature, extra-axial CSF coefficient decision, third-ventricle raw-threshold policy, and Chiari calibration status. | Clinician collaborator | Go only after unresolved source checks are marked reviewed |
...
| source_data_final_lock.md            | Clinician signoff packet for Dovjak, Woitek countersignature, extra-axial CSF, third-ventricle raw-threshold policy, and Chiari II / ONTD calibration.                                                         | Radiologist PI                |
