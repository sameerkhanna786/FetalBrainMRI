...
3. No endpoint changes are allowed after endpoint freeze unless the manuscript
   labels the analysis as exploratory.
4. No diagnostic-accuracy or reader-study sample-size assumptions may change
   after the sample-size / precision plan is signed without a PI amendment.
5. Any code-version change after code-version freeze requires re-running the
   full validation export and recording the new commit hash.
6. Chiari II / ONTD research-mode status remains in force until local
